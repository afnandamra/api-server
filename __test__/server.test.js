...
describe('Movies Route CRUD Test', () => {
  it('read all from DataBase test on GET /movies when there is no data', async () => {
    const response = await request.get('/api/v1/movies');
    expect(response.status).toEqual(200);
    expect(response.body).toEqual('No Data Available');
  });
  it('create test on POST /movies', async () => {
    const response = await request.post('/api/v1/movies').send({
      type: 'Action',
      actor: 'Ryan Reynolds',
    });
    expect(response.status).toEqual(201);
    expect(response.body.actor).toEqual('Ryan Reynolds');
    id = response.body._id;
  });
  ...
  it('should be able to update data on PUT /movies', async () => {
    ...
    expect(response.status).toEqual(200);
    expect(response.body.actress).toEqual('Blake Lively');
  });
  ...
  it('should throw an error if the ID does not exist on DELETE /movies', async () => {
